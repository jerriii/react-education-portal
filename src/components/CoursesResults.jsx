...
const CoursesResults = () => {
  ...
  let total_content_shown;

  if (window.innerWidth >= 768 && window.innerWidth < 1280) {
    total_content_shown = 6;
  } else if (window.innerWidth >= 1280 && window.innerWidth < 1536) {
    total_content_shown = 9;
  } else if (window.innerWidth >= 1536) {
    total_content_shown = 12;
  } else {
    total_content_shown = 3;
  }
    ...
    <section className="flex flex-col bg-white rounded-lg px-5 w-full">
      <section className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 w-full">
          {allCourses.slice(0, total_content_shown).map((course) => (
            <div
              key={course.id}
            ...
            >
              <div className="w-28 h-28 absolute bg-white p-4 rounded-full flex items-center shadow-lg">
                <img src={course.logo} alt="imglogo" className="w-24 h-24" />
              </div>
              <div className="bg-[#D9D9D9] px-4 py-14 md:pb-4 md:pt-10 rounded-lg mt-20 h-full">
